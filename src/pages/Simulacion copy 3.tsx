...
const Simulacion = () => {
  ...
  // Separar en dos funciones distintas

// Para voz: usa SOLO ElevenLabs
...
// Para texto: usa SOLO Groq
const handleTextInput = useCallback(
  async (text: string) => {
    if (!text.trim()) return;

    // Add user message (sin métricas de audio)
    const newUserMessage: { role: "user" | "ai"; text: string; audioMetrics?: AudioMetrics } = {
      role: "user",
      text,
    };
    ...
    try {
      ...
      // Speak the AI response con Web Speech API
      if (speakRef.current) {
        speakRef.current(aiResponse);
        // Mark when AI finishes speaking
        const estimatedSpeakTime = aiResponse.split(" ").length * 300;
        setTimeout(() => {
...
// Modificar useVoiceInteraction para usar handleVoiceInput
const { isListening, isSpeaking, transcript, startListening, stopListening, speak, stopSpeaking, isSupported } =
  useVoiceInteraction({
    onTranscript: handleVoiceInput, // Usar handleVoiceInput
    language: "es-ES",
  });

// Modificar handleTextSubmit para usar handleTextInput
const handleTextSubmit = () => {
  if (!textInput.trim()) return;

  handleTextInput(textInput); // Usar handleTextInput
  setTextInput("");
};
